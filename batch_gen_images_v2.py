#!/usr/bin/env python3
"""
满分虎网站资讯文章配图批量生成脚本 v2
修复：正确匹配 .astro 文件中的 JSX prop 格式
"""
...
def update_article_image(astro_path: str, img_path: str):
    ...
    # Pattern: image="..." (full value including quotes, may span after = sign)
    # The prop may use single or double quotes; our files use double quotes.
    img_prop_pattern = re.compile(r'image="[^"]*"')

    new_prop = f'image="{img_path}"'

    if img_prop_pattern.search(content):
        # Replace existing image prop
        updated = img_prop_pattern.sub(new_prop, content)
    else:
        # No image prop exists, add it before the closing > of ArticleLayout opening tag
        # Strategy: find the opening <ArticleLayout ... > tag and insert image prop before the >
        tag_pattern = re.compile(r'(<ArticleLayout\b[^>]*)(>)')
        m = tag_pattern.search(content)
        if m:
            # Insert image prop before the closing >
            new_tag = m.group(1) + f'\n  image="{img_path}"' + m.group(2)
            updated = content[:m.start()] + new_tag + content[m.end():]
        else:
            print(f'  [!] Could not find <ArticleLayout> tag in {os.path.basename(astro_path)}')
            return
...
def update_index(slug_map: dict):
    ...
    for slug, img_filename in slug_map.items():
        # Case 1: current src is a local file (SVG or old PNG)
        content = re.sub(
        ...
        # Match src="https://images.unsplash.com/photo-... ..."
        unsplash_pattern = re.compile(rf'src="[^"]*photo-[^"]*"')
        if unsplash_pattern.search(content):
            content = unsplash_pattern.sub(f'src="/images/news/{img_filename}"', content)
