...
"🔥","💧","🪨","🌪️","⚡",
"❄️","👻","🌟","🌿","⭐"];
var PN=["紫瞳三眼喵","独眼马铃薯","蘑菇精","方块眼仔","蓝皮狐","黑白团子","像素企鹅","夜枭眼","腮帮大鼓","松果怪","迷你小龙","彩虹马","独角兽","鬃毛狮子","花仙子","叶落","珊瑚女","冰墩墩","赤焰狐","梅花鹿","双生樱桃","蜡滴怪","硬邦邦","旋风","闪电侠","寒冰","幽灵飘飘","绿锥仙人","草太郎","流星仔"];
...
function pwr(s,lv){
return Math.round((s.atk*1.5+s.def*0.8+s.spd*0.7+s.int*0.5+s.luk*0.3)*(1+(lv-1)*0.03))}
...
'<div class=ps><div class=walk-area><img id=homePetGif class=pg src="/images/pets/pet_'+String(sid+1).padStart(2,"0")+'_'+(window._pf||"walk")+'.gif" alt=""></div><div class=pn>'+(PN[sid]||'精灵 #'+(sid+1))+(evo?'<span class="evo-badge">'+evo.stage+'</span>':'')+'</div><span class="pt badge '+(mx?"bg-gold":"bg-blue")+'">Lv.'+lv+(mx?" 🏆":"")+'</span></div>'+
...
function cL(n){var p=st.people[n],idx=p.currentPetIdx,nd=xfl(p.petLevels[idx]);while(p.petXP[idx]>=nd){p.petXP[idx]-=nd;p.petLevels[idx]++;var ne=getEvo(p.petLevels[idx]);var oe=getEvo(p.petLevels[idx]-1);if(ne&&ne!==oe)toast("🌟 "+(PN[p.petSlots[idx]]||"精灵 #"+(p.petSlots[idx]+1))+" 进化至 "+ne.stage+"！");playSound("levelup");p.happiness=Math.min(100,p.happiness+10);if(p.petLevels[idx]>=100){var ni=rb(n);if(ni!==null){p.pendingBoxes.push(ni)}}nd=xfl(p.petLevels[idx])}sv()}
...
function rColl(){var n=st.tn,p=st.people[n],un=new Set(),cur=-1;if(p){p.petSlots.forEach(function(s){un.add(s)});if(p.petSlots[p.currentPetIdx]!==undefined)cur=p.petSlots[p.currentPetIdx]}document.getElementById("collC").textContent=un.size+"/"+TP;document.getElementById("collGrid").innerHTML=Array.from({length:TP},function(_,i){var u=un.has(i),c=i===cur;return '<div class="ccell '+(c?"cur":u?"unlocked":"lock")+'"'+(u?' onclick="showPetDetail('+i+')"':'')+'><span class=ce>'+(u?pi(i):"❓")+'</span><span class=cn>'+(PN[i]||"#"+(i+1))+'</span></div>'}).join("")}
